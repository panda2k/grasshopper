...
#give me an attend button with the text "Attend" in the center of the div, with black background and white text, rounded borders, and the button is at the end of the page.
#also give me a state variable that changes the text of the button to "Attending" when clicked, and removes mainEvent() from the page
def attendButton() -> rx.Component:    
    icon = rx.cond(GlobalState.button_text=="Attend", rx.icon("arrow-up-right"),rx.icon("thumbs-up"))
    return rx.box(
        rx.button(
            rx.flex(
                rx.text(GlobalState.button_text, size="5"),
                icon,
                align="center",
                spacing='3'
            ),
            ...
            on_click=GlobalState.toggle_text()  # Call handle_click function when the button is clicked
        ),
        style={"width": "100%"}
    )
